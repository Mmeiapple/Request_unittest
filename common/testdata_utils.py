...
class TestdataUtils():
    def __init__(self, sheet_name='Sheet1', test_data_path=path_data_file):
        ...
        self.test_data_mysql = SqlUtils().get_mysql_case_info()
    ...
    def get_testcase_data_dict_mysql(self):
        all_case = {}
        for i in self.test_data_mysql:
            all_case.setdefault(i['测试用例编号'], []).append(i)
        return all_case

    def get_testcase_data_list_mysql(self):
        mysql_data = self.get_testcase_data_dict_mysql()
        data_list = []
